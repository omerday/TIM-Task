...
class Pathway(object):
    ...
    def __init__(self, ip, port_number, timeout=5., verbose=True, buffer_size=1024):
        ...
        self.command_codes = {
            ...
            5: 'STOP',
            6: 'ABORT',
            7: 'YES',  # used to start increasing the temperature
            8: 'NO'  # used to start decreasing the temperature
        }
        self.response_codes = {
            0: 'RESULT_OK',
        ...
        try:
            ...
            raise IOError(
                'Cannot establish connection, check IP and port number is correct and the host computer is on.')
    ...
    def _format_command(self, command, protocol):
        ...
        """
        # converts an integer to its binary representation of length 32
    ...
    def _decode(self, data_int, whichtime):
        ...
        dat_bin = [bin8(dat_int[i]) for i in
                   range(0, self.segmentation_points[whichtime][1] - self.segmentation_points[whichtime][0])]
        dat_bin.reverse()
    ...
    def poll_for_change(self, to_watch, desired_value, poll_interval=.5, poll_max=-1, verbose=False, server_lag=1.,
                        reuse_socket=False):
        """
    ...
    def set_pulse(self, binary_code):
        """Set the Medoc device to output a heat pulse with the given binary code."""
        command = f'START {binary_code}'
        self.call(command)
